...
void
YM_envelope_KONKOFF(YM2151Operator *op, int v)
{
	if (v & 0x08) /* M1 */
		KEY_ON(op + 0, 1)
	else
...
void
YM_set_connect(YM2151Operator *om1, int cha, int v)
{
...
void
YM_refresh_EG(YM2151Operator *op)
{
	uint32_t kc;
	uint32_t v;
...
signed int
YM_op_calc(YM2151Operator *OP, unsigned int env, signed int pm)
{
	uint32_t p;
...
signed int
YM_op_calc1(YM2151Operator *OP, unsigned int env, signed int pm)
{
	uint32_t p;
	int32_t  i;
...
void
YM_chan_calc(unsigned int chan)
{
	YM2151Operator *op;
	unsigned int    env;
	uint32_t        AM = 0;
...
*/
void
YM_stream_update(uint16_t *stream, uint32_t samples)
{
	uint32_t i;
	int32_t  outl, outr;
