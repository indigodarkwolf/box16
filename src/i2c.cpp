...
uint8_t i2c_read(uint8_t device, uint8_t offset)
{
	...
	LOG_PRINT(1, "I2C READ(${:02X}:${:02X}) = ${:02X}\n", device, offset, value);
	return value;
}
...
void i2c_write(uint8_t device, uint8_t offset, uint8_t value)
{
	...
	LOG_PRINT(1, "I2C WRITE ${:02X}:${:02X}, ${:02X}\n", device, offset, value);
}
		...
		if (state != STATE_STOP && i2c_port.clk_in == 1 && old_i2c_port.clk_in == 0) {
			...
			if (state < 8) {
				if (read_mode) {
					...
					value <<= 1;
					LOG_PRINT(4, "I2C OUT#{:d}: {:d}\n", state, i2c_port.data_out);
					state++;
				} else {
					LOG_PRINT(4, "I2C BIT#{:d}: {:d}\n", state, i2c_port.data_in);
					value <<= 1;
				...
				if (read_mode) {
					...
					if (nack) {
						LOG_PRINT(3, "I2C OUT DONE (NACK)\n");
						count     = 0;
						read_mode = false;
					} else {
						LOG_PRINT(3, "I2C OUT DONE (ACK)\n");
					...
					if (ack) {
						LOG_PRINT(3, "I2C ACK({:d}) ${:02X}\n", count, value);
						i2c_port.data_out = 0;
						count++;
					} else {
						LOG_PRINT(3, "I2C NACK({:d}) ${:02X}\n", count, value);
						count     = 0;
						read_mode = false;
					}
				}
