...
#include "../vendor/SDL2-2.0.10/include/SDL_scancode.h"
...
uint16_t ps2_scancode_from_SDL_Scancode(SDL_Scancode scancode)
{
	switch (scancode) {
...
int main()
{
	uint16_t SDL_to_PS2_table[SDL_NUM_SCANCODES] = {0};

	cout << "static const uint16_t SDL_to_PS2_table[] = {";

    for(int i=0; i<SDL_NUM_SCANCODES; ++i) {
        if((i & 15) == 0) {
            cout << endl << "\t";
        }
        uint16_t keycode = (uint16_t)ps2_scancode_from_SDL_Scancode((SDL_Scancode)i);
        cout << "0x" << std::setfill('0') << std::setw(4) << hex << keycode << ", ";
    }
    cout << endl << "};" << endl;
